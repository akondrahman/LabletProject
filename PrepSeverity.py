...
def printTacticSeverityMapping(full_df, DS_NAME): 
    print('*'*100)     
    print(DS_NAME)
    print('*'*100)     
    cve_tactic_dict = {}
    ...
    for severity in severity_list:
        if severity != 'NOT_FOUND':
            ...
            for tactic in severity_tactics: 
                    ...
                    cve_prop             = round(float(cve_count)/float(len(severity_cves)) , 5) * 100 
...
if __name__=='__main__':
    ...
    printTacticSeverityMapping(merged_dataframe, DATASET_NAME)   
    dumpSeverityPlottingData(merged_dataframe, DATASET_NAME)  
